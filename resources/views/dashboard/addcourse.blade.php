...
<div class="p-10 sm:ml-64">
  <div class="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700 mt-20">
    ...
    <div class="col-lg-8">
      <form method="post" action="{{ route('sponsors.take') }}" class="mb-5" enctype="multipart/form-data">
        @csrf

        <!-- Select Course -->
        <div class="max-w-sm mb-3">
          <label for="sponsor" class="form-label block mb-2 text-large font-medium text-gray-900 dark:text-black">Select Course</label>
          <select name="sponsor_id" id="sponsor" class="form-select @error('sponsor_id') is-invalid @enderror bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-100 dark:border-gray-600 dark:placeholder-gray-400 dark:text-black dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
            <option value="">Select a Course</option>
            @foreach ($sponsors as $sponsor)
            <option value="{{ $sponsor->id }}" data-schedule="{{ $sponsor->schedule }}" data-price="{{ $sponsor->price }}">{{ $sponsor->title }}</option>
            @endforeach
          </select>
          @error('sponsor_id')
